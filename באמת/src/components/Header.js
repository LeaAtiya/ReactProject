...
export default function Header() {
  ...
  return (
    <div>
      ...
      >
        <BottomNavigationAction label="דף הבית" icon={<Home />} onClick={() => { navigate("/homepage"); setValue(0) }} value={0} />
        ...
        <BottomNavigationAction label="הוספת מתכון" icon={<LibraryAddIcon />} onClick={() => { navigate("/addRecipe"); setValue(3) }} value={3} />
        <BottomNavigationAction label="הוספת קטגוריה" icon={<DataSaverOnIcon />} onClick={() => { navigate("/addCategory"); setValue(4) }} value={4} />
        <BottomNavigationAction label="רשימת קניות" icon={<ShoppingCartIcon />} onClick={() => { navigate("/shoppingList"); setValue(5) }} value={5} />
